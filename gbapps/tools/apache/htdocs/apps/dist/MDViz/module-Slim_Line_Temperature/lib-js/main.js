...
var dmsTempData;
var textinputSerial;
var titleStringDefault = "Slim Line Temperature";
var zoomStartIndexLast;
var zoomEndIndexLast;
...
function setTitleStringSub() {
	var dateGantryStart = undefined;
	var dateGantryEnd = undefined;
	if (gantryTempData !== undefined) {
		var m = gantryTempData.metadata;
		dateGantryStart = m.dateStart;
		dateGantryEnd = m.dateEnd;
	}
	var dateRoomStart = undefined;
	var dateRoomEnd = undefined;
	if (roomTempData !== undefined) {
		var m = roomTempData.metadata;
		dateRoomStart = m.dateStart;
		dateRoomEnd = m.dateEnd;
	}

	var dateStart = getEarlierDate(dateGantryStart, dateRoomStart);
	var dateEnd = getEarlierDate(dateGantryEnd, dateRoomEnd);
	...
	// chart2.titles[0].text = titleString;
}
...
function showMetadataAll() {
	if (gantryTempData !== undefined) {
		...
		document.getElementById('gantryDateStart').innerHTML = m.dateStart.toLocaleDateString();
		document.getElementById('gantryDateEnd').innerHTML = m.dateEnd.toLocaleDateString();
		// document.getElementById('gantryDateStart').innerHTML = convertToDateStringISOLocalTimezone(m.dateStart);
		// document.getElementById('gantryDateEnd').innerHTML = convertToDateStringISOLocalTimezone(m.dateEnd);
		document.getElementById('gantryNumMeas').innerHTML = m.numMeas;	
	}
	if (roomTempData !== undefined) {
		var m = roomTempData.metadata;
		document.getElementById('roomFilename').innerHTML = m.filename;
		document.getElementById('roomDateStart').innerHTML = m.dateStart.toLocaleDateString();
		document.getElementById('roomDateEnd').innerHTML = m.dateEnd.toLocaleDateString();
		document.getElementById('roomNumMeas').innerHTML = m.numMeas;
	}
...
function selectParam(idSelectBox, idxGraph, chartInput, doRefresh) {
	var e = document.getElementById(idSelectBox);
	var param = e.options[e.selectedIndex].value;
	console.log("param=", param)

	if (param == "(none)") {
		chartInput.graphs[idxGraph].hidden = true;
	}
	else {
		chartInput.graphs[idxGraph].valueField = param;
		chartInput.graphs[idxGraph].hidden = false;
		chartInput.graphs[idxGraph].valueAxis.title = param;

		var idxColumn = getColumnIndex(columnDef, param);
		//showMessage(param + "--" + idxColumn, "#ffff00");
		chartInput.graphs[idxGraph].valueAxis.minimum = columnDef[idxColumn].min;
		chartInput.graphs[idxGraph].valueAxis.maximum = columnDef[idxColumn].max;
	}
	...
	if (doRefresh) {
		...
		chartInput.validateData();

		// restore zoom window
		chartInput.zoomToIndexes(z1, z2);
	}
}
...
function preselectPlotParameters() {
	if (gantryTempData !== undefined) {
		selectOption('tube_param1', 'Gantry_Temp1_degC');
	}
	if (roomTempData !== undefined) {
		selectOption('tube_param2', 'Room_Temp1_degC');
		// selectOption('tube_param3', 'Room_Temp2_degC');
	}
	if (dmsTempData !== undefined) {
		selectOption('tube_param3', 'DMS_Temp1_degC');
	}
}
...
function convertToJointFormat(logdata) {
	...
	for (var i=0; i<logdata.length; i++) {
		...
		var recordNew = {
			datetime: NaN,
			Gantry_Temp1_degC: NaN,
			Gantry_Temp2_degC: NaN,
			Gantry_Temp3_degC: NaN,
			Room_Temp1_degC: NaN,
			Room_Temp2_degC: NaN,
			DMS_Temp1_degC: NaN,
			DMS_Temp2_degC: NaN,
...
function makeColumnDefinition() {
	...
	columnDef = columnDef.concat(roomTempParser.columnDef.slice(1,3));
...
function makeFullData() {
	...
	if (roomTempData !== undefined) {
		var temp = convertToJointFormat(roomTempData.logdata);
		fullData = fullData.concat(temp);
	}
	if (dmsTempData !== undefined) {
...
function mergeRecords(r1, r2) {
	var ret = {
		...
		Gantry_Temp1_degC	: mergeNumbers(r1.Gantry_Temp1_degC,	r2.Gantry_Temp1_degC),
		Gantry_Temp2_degC	: mergeNumbers(r1.Gantry_Temp2_degC,	r2.Gantry_Temp2_degC),
		Gantry_Temp3_degC	: mergeNumbers(r1.Gantry_Temp3_degC,	r2.Gantry_Temp3_degC),
		Room_Temp1_degC	: mergeNumbers(r1.Room_Temp1_degC,	r2.Room_Temp1_degC),
		Room_Temp2_degC	: mergeNumbers(r1.Room_Temp2_degC,	r2.Room_Temp2_degC)
	};
	return ret;
}
...
function initializeMain() {
	...
	document.getElementById('fileinputRoom').addEventListener(
		'change', callbackRoomFileinput, false);
	document.getElementById('fileinputDMS').addEventListener(
		'change', callbackDMSFileinput, false);

	gantryTempParser = new SlimLineGantryTempParser();
	roomTempParser = new SlimLineRoomTempParser();
...
function callbackButtonExportPNG() {
	...
	var exp2;
	if (chart2.AmExport === undefined) {
		exp2 = new AmCharts.AmExport(chart2);
		exp2.init();
	}
...
*/
	...
	// initialize canvas with dummy call
	chart.AmExport.output({format:'dummy',output:''})
	chart2.AmExport.output({format:'dummy',output:''})
	// "data:image/png;base64,iVBORw0KGgoAAAANSUh..."
	var part1 = chart.AmExport.canvas.toDataURL('image/png');
	var part2 = chart2.AmExport.canvas.toDataURL('image/png');
	part2 = part2.split(",")[1];
	var pngData = part1 + part2;
	// var pngData = part1;
	// <img src="data:image/png;base64,iVBORw0KGgo">
	var htmlcode = '<img height="99" src="' + pngData + '">';
	console.log("htmlcode=",htmlcode)
	document.getElementById('testpng').innerHTML = htmlcode;
	// document.getElementById('testpng').innerHTML = '457567567';

	const canvas = document.createElement('canvas');
	const ctx = canvas.getContext('2d');
	canvas.width = 200;
	canvas.height = 100;
	const blob1 = chart.AmExport.generateBlob(part1, 'image/png');
	const img1 = createImageBitmap(blob1);
	console.log("img1=", img1)
	ctx.drawImage(img1, 200, 100);
}
