...
function callbackReadingAllDone() {
	...
	// }
	listTabObjects["tabMeasurements"].initialize();
...
function callbackOnLoad(event) {
	...
	try {
		...
		// parse serial from filename
		// console.log("event.target=", event.target.filename);
		var serial = parseSerialFromURL(event.target.filename);
		document.getElementById("systemSerial").innerHTML = serial;
	}
	catch(err) {
