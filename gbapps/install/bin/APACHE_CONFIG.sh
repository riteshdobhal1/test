...
if [ -n $APACHE_HOST ];then
	if [ -e $CONFIGURATION_FILE -a -e $PERL_SCRIPT ];then
		if [ $REMOTE_APACHE == "yes" ];then
			scp $CONFIGURATION_FILE $PERL_SCRIPT $APACHE_USER\@$APACHE_HOST:/tmp &> /dev/null
		else
			cp $CONFIGURATION_FILE $PERL_SCRIPT /tmp &> /dev/null
		fi
		if [ $? == 0 ];then
			if [ $REMOTE_APACHE == "yes" ];then
				ssh $APACHE_USER\@$APACHE_HOST "perl /tmp/$PERL_SCRIPT apache"
			else
				perl /tmp/$PERL_SCRIPT apache
			fi
			if [ $? != 0 ];then
			...
			failed_status "SCP to remote $APACHE_HOST failed"
                	exit 1
		fi
        ...
        failed_status "APACHE host entry missing server_dtls file"
        exit 1	
fi
ok_status
