...
// initialize
// ------------------------------------------------------------------------
function initializeModuleGoEMBLog() {
	// window height
	...
	eventLogParser = new GoEMBLogParser();
	// var dmsTempData;
...
// console.log('main.js: 56876586');
	...
	hideTab("tabFirmware");
	hideTab("tabSystemInfo");
	hideTab("tabISV59");

	// hide basic events
	document.getElementById('chartScanIntervals').style.display = 'none';

	// DEV_loadData()
}


function DEV_loadData() {
	function loadFileViaXMLHttpRequest(filepath) {
		// console.log("loadFileViaXMLHttpRequest start");
		var xmlHttp = typeof XMLHttpRequest != "undefined"
			? new XMLHttpRequest()
			: new ActiveXObject("Microsoft.XMLHTTP");

		xmlHttp.onreadystatechange = DEV_callbackOnReadyStateChange;
		xmlHttp.open("GET", filepath, true); // true for asynchronous
		xmlHttp.responseType = "text";
		xmlHttp.send(null);
	}

	function DEV_callbackOnReadyStateChange(evt) {
		// console.log(evt);
		var xmlHttp = evt.target;
		// console.log("readyState=" + xmlHttp.readyState + " : status=" + xmlHttp.status);
		if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
			// console.log(">>" + xmlHttp.responseText + "<<");
			// console.log(">>" + xmlHttp.responseText.substr(0,500) + "<<");
			readData(xmlHttp);
		}
		// console.log("callbackOnReadyStateChange end");
	}

	// var filepath = "CT_11061628_108069_aCTimas_2019-02-05T21-31_Environment_0205_125514.txt";
	var filepath = "CT_11061628_108069_aCTimas_2019-02-06T21-21_Environment_0206_125305.txt";
	console.log("filepath=", filepath);
	loadFileViaXMLHttpRequest(filepath);
}

function readData(xmlHttp) {
	var serial = parseSerialFromURL(xmlHttp.responseURL);
	console.log("serial=", serial);
	var parser = new GoEMBLogParser();
	var logdata = parser.parse(xmlHttp.responseText);
	// console.log("logdata=", logdata);
}

function parseSerialFromURL(url) {
	// console.log("url=", url);
	var serial = undefined;
	const regex = /CT_\d+_(\d+)/;
