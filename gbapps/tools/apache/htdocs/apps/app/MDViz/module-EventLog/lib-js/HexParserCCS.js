...
function HexParserCCS() {
	this.specification = [
		...
		// added 2020-04-02
		{ id:38,	pbyte:'p12',	scale:0.1,	unit:'degC',	name:'Room temperature' },
		{ id:42,	pbyte:'p12',	scale:1,	unit:'%',		name:'Room humidity' },
		{ id:45,	pbyte:'p12',	scale:1,	unit:'m^3/h',	name:'Air flow' },
	];

	// "(E 01 13 22 00 CD 00 05) Air outlet temperature monitoring"
	this.lookupCoolingType = {
...
HexParserCCS.prototype.getNamesAll = function() {
...
HexParserCCS.prototype.getSpecs = function(eventId) {
...
*/
HexParserCCS.prototype.parse = function(record, spec) {
...
HexParserCCS.prototype.parseSingle = function(record) {
	var specs = this.getSpecs(record.event_id);
	var listMeas = [];
	for (var j=0; j<specs.length; j++) {
		var meas = this.parse(record, specs[j]);
		listMeas = listMeas.concat(meas);
	}
	return listMeas;
}

HexParserCCS.prototype.parseAll = function(listEventLogObjects) {
	var measurements = [];
	var hashNames = {};
...
HexParserCCS.prototype.getUnit = function(name) {
	var specsAll = this.specification;
	for (var i=0; i<specsAll.length; i++) {
...
window.HexParserCCS = HexParserCCS;
