...
function SensationGantryTempParser() {
	// plot: 1=line, 2=indicator, 0=none
	this.columnDef = [
		/* 0 */	{ id: 'datetime',			plot: 0, min: 0, max: 0 },
		/* 1 */	{ id: 'Gantry_Temp_degC',	plot: 1, min: 0, max: 40 },
	];
...
*/
SensationGantryTempParser.prototype.parse = function(fileAsString, filename) {
...
SensationGantryTempParser.prototype.readMetadata = function(filename, logdata) {
...
SensationGantryTempParser.prototype.parseTimestamp = function(s) {
	...
	if (date.toString() === 'Invalid Date') {
		throw 'SensationGantryTempParser: Timestamp has unknown format.';
	}

	return date;
}

SensationGantryTempParser.prototype.parseDataLine = function(strInput) {
	var tokens = strInput.split(this.delim);
	if (tokens.length != this.numCols) {
		throw 'SensationGantryTempParser: Input file must have ' + this.numCols + ' columns.';
	}
...
SensationGantryTempParser.prototype.readLogdata = function(fileAsString) {
	...
	// no header in Sensation GantryTemp files
	var iFirst = 0;
...
SensationGantryTempParser.prototype.sortLogdata = function(logdata) {
